...
def conv1d_noraml(value, output_dim, k_w=2,
            stride=1, name='conv1d'):
    with tf.variable_scope(name):
        weights = weight('weights',
                        [k_w, value.get_shape()[-1], output_dim])
        conv = tf.nn.conv1d(value, weights, stride=stride, padding='VALID')
        biases = bias('biases', [output_dim])
        conv = tf.nn.bias_add(conv, biases)
        return conv
...
def max_pool(x, s=[2,2]):
    return tf.nn.max_pool(x, ksize=[1, s[0], s[1], 1], strides=[1, s[0], s[1], 1], padding='VALID')