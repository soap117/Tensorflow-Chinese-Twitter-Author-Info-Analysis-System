...
def train_career():
    ...
    drop_rate, blogs, blogs_vec, ans, loss, accuracy, _ = model.build(is_dropout=True)
    ...
    for i in range(epoch):
        ...
        sess.run(optim,
                 feed_dict={blogs: data_list, blogs_vec: data_list_words, ans: ans_list, drop_rate: 0.5})
        if i % 100 == 0:
            ac, ls = sess.run([accuracy, loss],
                              feed_dict={blogs: data_list, blogs_vec: data_list_words, ans: ans_list, drop_rate: 1.0})
        ...
        if i % 1000 == 0 and i > 0:
            ...
            for j in range(T):
                ...
                data_list_words = test_data_words[j * batch_size:(j + 1) * batch_size]
                ans_list = test_ans[j * batch_size:(j + 1) * batch_size, 0]
